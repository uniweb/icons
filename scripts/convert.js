...
async function convertFamily(familyCode) {
  ...
  // The index lists what this family HAS; it deliberately does not re-export
  // the icons themselves. Each icon module is a `export default "<svg…>"`, so a
  // barrel would have to name all ${success} of them — and most icon ids
  // (`a-arrow-down`) are not valid JS identifiers. Callers that want an icon
  // import it directly or go through `createLocalResolver`; callers that want
  // to know which icons exist (a picker, a validator) want exactly this list
  // and none of the SVG payload.
  //
  // A previous version emitted `export * from './*.js'` here. That is not
  // valid ESM — a wildcard is not a module specifier — so importing this file
  // threw ERR_MODULE_NOT_FOUND and failed any bundler that resolved it. It
  // would not have worked even if it parsed: `export *` does not re-export
  // default exports, which is all these modules have.
  iconNames.sort()
  const indexContent = `/**
 * ${config.displayName} Icons — manifest
 *
 * Names only. To load an icon:
 *   import zap from '@uniweb/icons/families/${familyCode}/zap.js'   // the SVG string
 *   import { createLocalResolver } from '@uniweb/icons/resolver'    // by { library, name }
 *
 * GENERATED by scripts/convert.js — do not edit.
 */

export const family = '${familyCode}'

export const names = ${JSON.stringify(iconNames, null, 2)}
